...
public class MyCallsAdapter extends ArrayAdapter<Model> {
	...
	void sendMail(String fileName) {
		...
		if (file.exists())
			sendIntent.putExtra(
					Intent.EXTRA_STREAM,
					Uri.parse("file://" + RecordFileHelper.getFilePath() + "/"
							+ RecordConstants.FILE_DIRECTORY + "/" + fileName));
		else
			sendIntent.putExtra(
					Intent.EXTRA_STREAM,
					Uri.parse("file://"
	...
	void startPlayExternal(String charSequence) {
		...
		if (file.exists())
			intentUri = Uri.parse("file://" + RecordFileHelper.getFilePath() + "/"
					+ RecordConstants.FILE_DIRECTORY + "/" + charSequence);
		else
