...
public class RecordActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences settings = this.getSharedPreferences(
				RecordConstants.LISTEN_ENABLED, 0);
	...
	protected void onResume() {
		if (updateExternalStorageState() == RecordConstants.MEDIA_MOUNTED) {
			final List<Model> listDir = RecordFileHelper.listFiles(this);
		...
		} else if (updateExternalStorageState() == RecordConstants.MEDIA_MOUNTED_READ_ONLY) {
			mScrollView2.setVisibility(TextView.VISIBLE);
			listView.setVisibility(ScrollView.GONE);
	...
	public static int updateExternalStorageState() {
		...
		if (Environment.MEDIA_MOUNTED.equals(state)) {
			return RecordConstants.MEDIA_MOUNTED;
		} else if (Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)) {
			return RecordConstants.MEDIA_MOUNTED_READ_ONLY;
		} else {
			return RecordConstants.NO_MEDIA;
		}
	}
...
//			startActivity(i);
	...
	private void setSharedPreferences(boolean silentMode) {
		...
		myIntent.putExtra("commandType",
				silentMode ? RecordConstants.RECORDING_DISABLED
						: RecordConstants.RECORDING_ENABLED);
